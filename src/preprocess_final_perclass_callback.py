...
# Current baseline:
# - keep ["person"] while debugging the pipeline.
# - later change to ["person", "chair", "car", "dog", "bottle"].
TARGET_CLASSES = ["person"]

# Current stage:
# - True  = all selected target classes are mapped to class 0: "object"
# - False = each target class gets its own YOLO class ID based on TARGET_CLASSES order
#
# For per-class metrics later, set:
# SINGLE_CLASS = False
SINGLE_CLASS = True
...
# Future 5-class balancing:
# Disabled for the current single-class person baseline.
# When using 5 classes, enable this to downsample only person-only training images.
...
def read_voc_index_mask(mask_path: Path) -> np.ndarray | None:
    """
    Read a VOC SegmentationObject mask as a 2D indexed array.
...
def mask_to_polygons(mask_path: Path, img_w: int, img_h: int, object_names: list[str]):
    ...
    - no resizing before contour extraction
    - use target instances only
...
def apply_train_balancing(train_ids: list[str], id_to_targets: dict[str, set[str]]) -> list[str]:
    """
    Optional future 5-class balancing.

    Downsamples only train images where person is the only selected target class.
    This avoids deleting useful co-occurrence images like person+chair or person+car.
    """
    if not BALANCE_PERSON_ONLY_TRAIN:
        return train_ids

    if "person" not in TARGET_CLASSES or len(TARGET_CLASSES) <= 1:
        print("ℹ️ Train balancing skipped: only use this for future multi-class experiments.")
        return train_ids
    ...
    print(
        f"⚖️ Train balancing enabled: dropped {dropped} person-only train images; "
        f"kept {len(balanced)}/{len(train_ids)}."
    )
    return balanced
...
def main():
    ...
    print(f"✅ Target filter {TARGET_CLASSES}: {len(valid_ids)} images match.")
    print(f"🏷️ YAML class names will be: {names_for_yaml()}")

    if not valid_ids:
        print("❌ No images found for the specified target classes. Exiting.")
        return
    ...
    for split_name, ids in splits.items():
        ...
        for img_id in tqdm(ids, leave=False):
            ...
            h, w = img.shape[:2]
            polygons, error = mask_to_polygons(src_mask, w, h, id_to_objects.get(img_id, []))
    ...
    print("\n🚀 Preprocessing complete. Run training with: python train.py")
