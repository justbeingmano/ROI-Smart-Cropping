...
# ============================================================
# RESUME / FINE-TUNE CONFIGURATION
# ============================================================

ROOT = Path(__file__).resolve().parent.parent
DATA_YAML = ROOT / "output" / "dataset" / "data.yaml"

# Previous YOLO26 run directory.
# This is the run that finished training but crashed during final validation.
PREVIOUS_RUN_DIR = (
    ROOT
    / "output"
    / "runs"
...
LAST_WEIGHTS = PREVIOUS_RUN_DIR / "weights" / "last.pt"

# ============================================================
# MODE SELECTION
# ============================================================
# Recommended:
# False = fine-tune from best.pt in a NEW run.
# This is safer because the previous training already early-stopped,
# and best.pt was the best checkpoint.
#
# True = exact Ultralytics resume from last.pt.
# Use only if you want to continue the interrupted run state exactly.
EXACT_RESUME = False

# Default: fine-tune from best.pt.
RESUME_WEIGHTS = BEST_WEIGHTS

RUN_NAME = "voc2012_5class_yolo26m_resume_finetune_img896"
...
# RTX 3080 dynamic batch fallback.
BATCH_CANDIDATES = [16, 12, 8, 4]

# Fine-tuning stage:
# We train less than the original run and use smaller LR.
EPOCHS = 120
PATIENCE = 80
...
def check_environment():
    ...
    # This was the original crash point.
    try:
        import torchvision.ops as tv_ops
        _ = tv_ops.nms
        print("✅ torchvision.ops.nms is available.")
    except (AttributeError, ImportError):
        print("⚠️ torchvision.ops.nms is NOT available.")
        print("🛠️ Patching torchvision.ops.nms with ultralytics.utils.nms.TorchNMS.nms...")
        try:
            ...
            print("✅ Successfully patched torchvision.ops.nms using TorchNMS fallback.")
        except Exception as e:
    ...
    if not DATA_YAML.exists():
        ...
        print("python src/preprocess_final.py")
        sys.exit(1)

    if EXACT_RESUME:
        if not LAST_WEIGHTS.exists():
            print(f"❌ last.pt not found: {LAST_WEIGHTS}")
            sys.exit(1)
    else:
        if not RESUME_WEIGHTS.exists():
            print(f"❌ best.pt not found: {RESUME_WEIGHTS}")
            sys.exit(1)
...
class QualityWatchCallback:
    ...
    def __call__(self, trainer):
        ...
        recommendations = {
            ...
            "reason": "weak_metrics" if clearly_weak else "plateau",
            "recommended_next_edits": [
                "If fine-tuning does not improve, use the original best.pt checkpoint.",
                "If recall is weak, try mosaic=0.0 in a separate run.",
                "If mAP50 is good but mAP50-95 is weak, keep high IMG_SIZE and train longer.",
                "If bottle/chair underperform, increase their upsample factors in preprocessing.",
                "If YOLO26 underperforms, compare with yolov8m-seg using the same dataset.",
            ],
        }
...
def train_once(batch_size: int):
    print("\n" + "=" * 70)
    print(f"🚀 Starting resume/fine-tune attempt with batch={batch_size}, imgsz={IMG_SIZE}")
    print(f"Mode: {'EXACT RESUME FROM LAST.PT' if EXACT_RESUME else 'FINE-TUNE FROM BEST.PT'}")
    print("=" * 70)

    if EXACT_RESUME:
        model = YOLO(str(LAST_WEIGHTS))

        # Exact resume uses previous Ultralytics training state.
    ...
    results = model.train(
        ...
        # Smaller LR because we are fine-tuning from best.pt.
        optimizer="AdamW",
        lr0=5e-5,
        lrf=0.01,
        cos_lr=True,
        ...
        # Gentler augmentation for resume/fine-tuning.
        degrees=5.0,
        scale=0.35,
        fliplr=0.5,
...
def evaluate_best_checkpoint(model, results, used_batch: int):
    print("\n" + "=" * 70)
    print("🏆 RESUME/FINE-TUNE COMPLETE")
    print("=" * 70)
    ...
    print("\n📈 FINAL TEST SEGMENTATION PERFORMANCE:")
    print(f"   • Resume mode   : {'exact resume' if EXACT_RESUME else 'fine-tune from best.pt'}")
    print(f"   • Used batch    : {used_batch}")
    print(f"   • Image size    : {IMG_SIZE}")
    print(f"   • Precision (M): {precision_m:.4f}")
    print(f"   • Recall (M)   : {recall_m:.4f}")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("🚀 YOLO26 Resume / Fine-Tune Training")
    print("=" * 70)
    print(f"Data YAML        : {DATA_YAML.resolve()}")
    print(f"Previous run dir : {PREVIOUS_RUN_DIR}")
    print(f"Best weights     : {BEST_WEIGHTS}")
    print(f"Last weights     : {LAST_WEIGHTS}")
    print(f"Exact resume     : {EXACT_RESUME}")
